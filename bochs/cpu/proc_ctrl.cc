...
static const Bit32u BX_POLY_ABI_BRIDGE_FLAG_DESCRIPTOR_IMPORTS = (1U << 6);
...
static const Bit64u BX_POLY_IMPORT_X86_DESCRIPTOR_SIZE = BX_CONST64(32);
  ...
  Bit32u return_kind, Bit32u arg_kind, Bit32u slot)
{
  ...
  BX_INFO(("poly_ud: pcall signature mode=%u slot=%u kind=%u target=%llx return=%llx",
  ...
  bx_address return_rip)
{
  ...
  if (R12 != 0 && bx_poly_import_uses_descriptor_args(import_id)) {
    bx_address x86_stack_base = bx_poly_return_cookie_valid ?
      bx_poly_return_cookie_rsp : RSP;
    ...
    if (target != 0 && trampoline != 0) {
      ...
      const bool descriptor_stack_from_memory =
        (descriptor_flags &
          BX_POLY_IMPORT_X86_DESCRIPTOR_STACK_FROM_MEMORY) != 0;
      ...
      else if (descriptor_stack_arg_qwords > 0 && !descriptor_stack_from_memory &&
          !descriptor_stack_from_gpr0 &&
          mode == BX_POLY_MODE_RAW_RISCV) {
        mapped = read_poly_riscv_reg(16, &arg6) &&
          read_poly_riscv_reg(17, &arg7);
      }
      else if (descriptor_stack_arg_qwords > 6 &&
          descriptor_stack_from_gpr0 && mode == BX_POLY_MODE_RAW_AARCH64) {
        mapped = read_poly_aarch64_reg(6, &arg6) &&
          read_poly_aarch64_reg(7, &arg7);
      }
      else if (descriptor_stack_arg_qwords > 6 &&
          descriptor_stack_from_gpr0 && mode == BX_POLY_MODE_RAW_RISCV) {
        mapped = read_poly_riscv_reg(16, &arg6) &&
          read_poly_riscv_reg(17, &arg7);
      }
      if (!mapped)
        return false;
      bx_address foreign_rsp = RSP;
      Bit64u stack_memory_start = 0;
      if (descriptor_stack_from_gpr0) {
        Bit64u gpr_args[8] = { arg0, arg1, arg2, arg3, arg4, arg5,
          arg6, arg7 };
        Bit64u gpr_stack_qwords = descriptor_stack_arg_qwords < 8 ?
          descriptor_stack_arg_qwords : 8;
        for (Bit64u n = 0; n < gpr_stack_qwords; n++)
          stack_args[n] = gpr_args[n];
        stack_memory_start = gpr_stack_qwords;
      }
      else if (!descriptor_stack_from_memory) {
        if (descriptor_aarch64_sret_x8) {
          if (descriptor_stack_arg_qwords > 0)
            stack_args[0] = arg5;
          if (descriptor_stack_arg_qwords > 1)
            stack_args[1] = arg6;
          if (descriptor_stack_arg_qwords > 2)
            stack_args[2] = arg7;
          stack_memory_start = 3;
        }
        else {
          if (descriptor_stack_arg_qwords > 0)
            stack_args[0] = arg6;
          if (descriptor_stack_arg_qwords > 1)
            stack_args[1] = arg7;
          stack_memory_start = 2;
        }
      }
      for (Bit64u n = stack_memory_start; n < descriptor_stack_arg_qwords; n++) {
        stack_args[n] = read_virtual_qword(BX_SEG_REG_SS,
          foreign_rsp + (bx_address) ((n - stack_memory_start) * 8));
      }
      bx_poly_commit_reg_state(BX_CPU_THIS_PTR cr3, MSR_FSBASE,
        bx_poly_current_state_key(foreign_rsp));
      if (bx_poly_import_x86_return_top >= BX_POLY_IMPORT_RETURN_DEPTH)
      ...
      frame->rip = return_rip;
      frame->rsp = foreign_rsp;
      frame->import_id = import_id;
      frame->descriptor_flags = descriptor_flags;
      frame->alias_valid = true;
      frame->alias[0] = RDI;
      frame->alias[1] = RSI;
      frame->alias[2] = RDX;
...
bool BX_CPU_C::execute_poly_raw_aarch64(Bit32u insn, bx_address pc)
{
  ...
  if (insn == BX_POLY_AARCH64_CTRL_X86_ESCAPE) {
    ...
    bx_poly_mode_switch_count++;
    RIP = next_rip;
    BX_INFO(("poly_raw: aarch64 polyctrl escape to x86"));
    return true;
  }

  if (insn == BX_POLY_AARCH64_CTRL_TRAP_RETURN) {
    BX_INFO(("poly_raw: aarch64 polyctrl trap return"));
  ...
  if (insn == BX_POLY_AARCH64_CTRL_RISCV_SWITCH) {
    ...
    bx_poly_mode_switch_count++;
    RIP = next_rip;
    BX_INFO(("poly_raw: aarch64 polyctrl switch to riscv"));
    return true;
  }
  ...
  if (insn == BX_POLY_AARCH64_CTRL_SWITCH_MODE) {
    ...
    BX_INFO(("poly_raw: aarch64 generic switch frontend=%u mode=%u target=%llx",
...
bool BX_CPU_C::execute_poly_raw_riscv(Bit32u insn, bx_address pc)
{
  ...
  if (insn == BX_POLY_RISCV_CTRL_X86_ESCAPE) {
    ...
    bx_poly_mode_switch_count++;
    RIP = next_rip;
    BX_INFO(("poly_raw: riscv polyctrl escape to x86"));
    return true;
  }
  ...
  if (insn == BX_POLY_RISCV_CTRL_AARCH64_SWITCH) {
    ...
    bx_poly_mode_switch_count++;
    RIP = next_rip;
    BX_INFO(("poly_raw: riscv polyctrl switch to aarch64"));
    return true;
  }
  ...
  if (insn == BX_POLY_RISCV_CTRL_SWITCH_MODE) {
    ...
    bx_poly_mode_switch_count++;
    RIP = (bx_address) target;
    BX_INFO(("poly_raw: riscv generic switch frontend=%u mode=%u target=%llx",
      frontend_id, target_mode, (unsigned long long) target));
    return true;
  }
  ...
  if (opcode0 == 0x0f && opcode1 == 0x3a && opcode2 == 0xfc) {
    ...
    bx_address next_rip = PREV_RIP + 4;
      ...
      if (op == 0x04) {
        ...
        BX_CPU_THIS_PTR async_event |= BX_ASYNC_EVENT_STOP_TRACE;
        RIP = target_rip;
        BX_INFO(("poly_op: x86 poly opcode pswitch frontend=%u mode=%u target=%llx",
...
void BX_CPP_AttrRegparmN(1) BX_CPU_C::CPUID(bxInstruction_c *i)
{
  ...
  if (BX_CPU_THIS_PTR poly_feature_enabled && EAX == BX_POLY_CPUID_BASE + 2) {
    if (ECX == 0) {
      ...
      RCX = BX_POLY_AARCH64_CTRL_RISCV_CALL_FP64_STACK;
      RDX = BX_POLY_RISCV_CTRL_AARCH64_CALL_FP64_STACK;
    }
    else if (ECX == 4) {
      RAX = BX_POLY_AARCH64_CTRL_TRAP_RETURN;
      RBX = BX_POLY_RISCV_CTRL_TRAP_RETURN;
      RCX = BX_POLY_AARCH64_CTRL_RISCV_CALL_VEC128_U32;
      RDX = BX_POLY_RISCV_CTRL_AARCH64_CALL_VEC128_U32;
    }
    else if (ECX == 5) {
  ...
  if (BX_CPU_THIS_PTR poly_feature_enabled && EAX == BX_POLY_CPUID_BASE + 9) {
    ...
    RBX = BX_POLY_ABI_BRIDGE_FLAG_X86_SYSV_TO_AAPCS64 |
          ...
          BX_POLY_ABI_BRIDGE_FLAG_FP64_STACK |
          BX_POLY_ABI_BRIDGE_FLAG_DESCRIPTOR_IMPORTS |
          BX_POLY_ABI_BRIDGE_FLAG_TLS_BASE |
          BX_POLY_ABI_BRIDGE_FLAG_USER_DESCRIPTORS |
    ...
    RDX = (Bit32u) BX_POLY_IMPORT_X86_DESCRIPTOR_SIZE |
          ((Bit32u) BX_POLY_IMPORT_CALL_STRIDE << 16);
    BX_NEXT_INSTR(i);
    return;
  }
