...
static void bx_poly_record_architectural_trap(Bit32u reason, Bit32u mode, Bit32u number,
  Bit32u selector, bx_address pc, Bit64u arg0, Bit64u arg1, Bit64u arg2,
  ...
  bx_address return_rip)
{
  ...
  else if (import_id == BX_POLY_IMPORT_FUNC_ERRNO_LOCATION) {
    if (bx_poly_foreign_tls_base == 0)
      return false;
  ...
  else if (import_id == BX_POLY_IMPORT_FUNC_GETENV) {
    result = 0;
    op_name = "getenv";
  }
  ...
  else if (import_id == BX_POLY_IMPORT_FUNC_ATEXIT) {
    result = 0;
    op_name = "atexit";
  }
  else if (import_id == BX_POLY_IMPORT_FUNC_CXA_ATEXIT) {
    result = 0;
    op_name = "__cxa_atexit";
  }
...
bool BX_CPU_C::execute_poly_raw_aarch64(Bit32u insn, bx_address pc)
{
  ...
  if ((insn & 0xffe0001f) == 0xd4200000) {
    ...
    if (!read_poly_aarch64_reg(2, &arg1) || !read_poly_aarch64_reg(3, &arg2))
      return false;
    handle_poly_libcall("aarch64", "brk", libcall_id, libcall_id, pc, arg1,
      arg2);
    RIP = next_rip;
    return true;
  }
...
bool BX_CPU_C::execute_poly_raw_riscv(Bit32u insn, bx_address pc)
{
  ...
  if (insn == 0x00100073) {
    ...
    if (!read_poly_riscv_reg(17, &libcall_id) ||
      ...
      return false;
    handle_poly_libcall("riscv", "ebreak", (Bit32u) libcall_id, 0, pc, arg1,
      arg2);
    RIP = next_rip;
    return true;
  }
    ...
    if (funct3 == 0x4) {
      ...
      if (high && rs2 == 0 && rd == 0) {
        ...
        if (!read_poly_riscv_reg(17, &libcall_id) ||
            ...
            !read_poly_riscv_reg(13, &arg2))
          return false;
        handle_poly_libcall("riscv", "c.ebreak", (Bit32u) libcall_id, 0, pc,
          arg1, arg2);
        RIP = next_rip;
        return true;
      }
...
bool BX_CPU_C::handle_poly_libcall(const char *arch_name, const char *trap_name,
  Bit32u libcall_id, Bit32u trap_selector, bx_address trap_pc, Bit64u arg1,
  Bit64u arg2)
{
  ...
  if (opcode0 == 0x0f && opcode1 == 0x24) {
    ...
    if (magic0 == 'P' && magic1 == 'O' && magic2 == 'L' &&
      ...
      if (op >= 0x50 && op <= 0x5a) {
        ...
        RIP = next_rip;
        BX_INFO(("poly_ud: trap status op=0x%02x id=%u reason=%u mode=%u number=%u selector=%u pc=%llx",
