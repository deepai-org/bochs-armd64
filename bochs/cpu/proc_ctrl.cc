...
bool BX_CPU_C::read_poly_aarch64_reg(Bit32u reg, Bit64u *value)
{
  ...
  case 0:
    *value = RAX;
    return true;
  case 2:
    *value = bx_poly_aarch64_x_valid[2] ? bx_poly_aarch64_x[2] : RDI;
    return true;
  case 31:
    *value = 0;
    return true;
...
bool BX_CPU_C::read_poly_riscv_reg(Bit32u reg, Bit64u *value)
{
  ...
  case 10:
    *value = RAX;
    return true;
  case 12:
    *value = bx_poly_riscv_x_valid[12] ? bx_poly_riscv_x[12] : RDI;
    return true;
  default:
    if (reg < 32) {
...
bool BX_CPU_C::execute_poly_raw_aarch64(Bit32u insn, bx_address pc)
{
  ...
  if ((insn & 0xffe00000) == 0xd2800000) {
    ...
    if (!write_poly_aarch64_reg(rd, imm16))
      return false;
    RIP = next_rip;
    BX_INFO(("poly_raw: emulated aarch64 movz x%u,#%u", rd, imm16));
    return true;
  }
  ...
  if ((insn & 0xff000000) == 0x91000000 || (insn & 0xff000000) == 0xd1000000) {
    ...
    if (!write_poly_aarch64_reg(rd, result))
      return false;
    RIP = next_rip;
    BX_INFO(("poly_raw: emulated aarch64 %s x%u,x%u,#%llu result=%llu", (insn & 0x40000000) ? "sub" : "add", rd, rn, (unsigned long long) imm, (unsigned long long) result));
    return true;
  }

  {
    ...
    if (op_name != 0) {
      if (!write_poly_aarch64_reg(rd, result))
        return false;
      RIP = next_rip;
      BX_INFO(("poly_raw: emulated aarch64 %s x%u,x%u,x%u result=%llu", op_name, rd, rn, rm, (unsigned long long) result));
      return true;
    }
  }
  ...
  if ((insn & 0xfc000000) == 0x14000000) {
    ...
    BX_INFO(("poly_raw: emulated aarch64 b offset=%lld", (long long) guest_offset));
    return true;
  }
  ...
  if ((insn & 0x7e000000) == 0x34000000) {
    ...
    if (taken) {
      ...
      RIP = (bx_address) ((Bit64s) pc + guest_offset);
      BX_INFO(("poly_raw: emulated aarch64 %s x0 taken offset=%lld", op ? "cbnz" : "cbz", (long long) guest_offset));
    }
    else {
      RIP = next_rip;
      BX_INFO(("poly_raw: emulated aarch64 %s x0 not-taken", op ? "cbnz" : "cbz"));
    }
    return true;
  }
  ...
  if (insn == 0xd65f03c0) {
    ...
    bx_poly_current_mode = BX_POLY_MODE_X86;
    RIP = ret_addr;
    BX_INFO(("poly_raw: emulated aarch64 ret rip=%llx", (unsigned long long) ret_addr));
    return true;
  }
  ...
  if ((insn & 0xffc00000) == 0xf9000000) {
    ...
    RIP = next_rip;
    BX_INFO(("poly_raw: emulated aarch64 str x%u,[x%u,#%u] addr=%llx value=%llu", rt, rn, imm12 << 3, (unsigned long long) addr, (unsigned long long) value));
    return true;
  }
  ...
  if ((insn & 0xffc00000) == 0xf9400000) {
    ...
    RIP = next_rip;
    BX_INFO(("poly_raw: emulated aarch64 ldr x0,[x%u,#%u] addr=%llx value=%llu", rn, imm12 << 3, (unsigned long long) addr, (unsigned long long) RAX));
    return true;
  }

  if ((insn & 0xffe0001f) == 0xd4000001) {
    Bit32u syscall_id = (insn >> 5) & 0xffff;
    Bit32u syscall_reg = (Bit32u) bx_poly_aarch64_x[8];
    Bit32u status_number = syscall_reg ? syscall_reg : syscall_id;
    return handle_poly_foreign_syscall("aarch64", "svc", "#", syscall_reg,
      status_number, syscall_id, RAX, bx_poly_aarch64_x[1], bx_poly_aarch64_x[2],
      bx_poly_aarch64_x[3], bx_poly_aarch64_x[4], bx_poly_aarch64_x[5], next_rip);
  }

  if ((insn & 0xffe0001f) == 0xd4200000) {
    Bit32u libcall_id = (insn >> 5) & 0xffff;
    handle_poly_libcall("aarch64", "brk", libcall_id, bx_poly_aarch64_x[1], bx_poly_aarch64_x[2]);
    RIP = next_rip;
    return true;
  }
...
bool BX_CPU_C::execute_poly_raw_riscv(Bit32u insn, bx_address pc)
{
  ...
  if ((insn & 0x0000707f) == 0x00000013) {
    ...
    if (!write_poly_riscv_reg(rd, (Bit64u) ((Bit64s) base + imm12)))
      return false;
    RIP = next_rip;
    BX_INFO(("poly_raw: emulated riscv addi x%u,x%u,%lld", rd, rs1, (long long) imm12));
    return true;
  }
  ...
  if ((insn & 0x0000007f) == 0x00000033) {
    ...
    if (op_name != 0) {
      if (!write_poly_riscv_reg(rd, result))
        return false;
      RIP = next_rip;
      BX_INFO(("poly_raw: emulated riscv %s x%u,x%u,x%u result=%llu", op_name, rd, rs1, rs2, (unsigned long long) result));
      return true;
    }
  }
  ...
  if ((insn & 0x0000707f) == 0x00000063 || (insn & 0x0000707f) == 0x00001063) {
    ...
    if (taken) {
      ...
      RIP = (bx_address) ((Bit64s) pc + guest_offset);
      BX_INFO(("poly_raw: emulated riscv %s taken offset=%lld", bne ? "bne" : "beq", (long long) guest_offset));
    }
    else {
      RIP = next_rip;
      BX_INFO(("poly_raw: emulated riscv %s not-taken", bne ? "bne" : "beq"));
    }
    return true;
  }
  ...
  if (insn == 0x00008067) {
    ...
    bx_poly_current_mode = BX_POLY_MODE_X86;
    RIP = ret_addr;
    BX_INFO(("poly_raw: emulated riscv jalr ret rip=%llx", (unsigned long long) ret_addr));
    return true;
  }
  ...
  if ((insn & 0x0000707f) == 0x00003023) {
    ...
    write_virtual_qword(BX_SEG_REG_DS, addr, value);
    RIP = next_rip;
    BX_INFO(("poly_raw: emulated riscv sd x%u,%u(x%u) addr=%llx value=%llu", rs2, imm, rs1, (unsigned long long) addr, (unsigned long long) value));
    return true;
  }
  ...
  if ((insn & 0x0000707f) == 0x00003003) {
    ...
    RAX = read_virtual_qword(BX_SEG_REG_DS, addr);
    RIP = next_rip;
    BX_INFO(("poly_raw: emulated riscv ld a0,%lld(x%u) addr=%llx value=%llu", (long long) imm12, rs1, (unsigned long long) addr, (unsigned long long) RAX));
    return true;
  }

  if (insn == 0x00000073) {
    Bit32u syscall_number = (Bit32u) bx_poly_riscv_x[17];
    return handle_poly_foreign_syscall("riscv", "ecall", "a7=", syscall_number,
      syscall_number, syscall_number, RAX, bx_poly_riscv_x[11], bx_poly_riscv_x[12],
      bx_poly_riscv_x[13], bx_poly_riscv_x[14], bx_poly_riscv_x[15], next_rip);
  }

  if (insn == 0x00100073) {
    handle_poly_libcall("riscv", "ebreak", (Bit32u) bx_poly_riscv_x[17], bx_poly_riscv_x[11], bx_poly_riscv_x[12]);
    RIP = next_rip;
    return true;
  }
...
bool BX_CPP_AttrRegparmN(1) BX_CPU_C::handle_poly_ud(bxInstruction_c *i)
{
  ...
  for (unsigned n = 0; n < 4; n++) {
    ...
    case 0x67: {
      ...
      if (bx_poly_current_mode == BX_POLY_MODE_AARCH64 &&
          ((insn & 0xff000000) == 0x91000000 || (insn & 0xff000000) == 0xd1000000)) {
        Bit32u rd = insn & 0x1f;
        Bit32u rn = (insn >> 5) & 0x1f;
        Bit64u base = 0;
        Bit64u imm = (insn >> 10) & 0xfff;
        ...
        if (!write_poly_aarch64_reg(rd, result))
          break;
        RIP = next_rip;
        BX_INFO(("poly_ud: emulated aarch64 %s x%u,x%u,#%llu result=%llu", (insn & 0x40000000) ? "sub" : "add", rd, rn, (unsigned long long) imm, (unsigned long long) result));
        return true;
      }
      if (bx_poly_current_mode == BX_POLY_MODE_AARCH64) {
        Bit32u rd = insn & 0x1f;
        ...
        if (op_name != 0) {
          if (!write_poly_aarch64_reg(rd, result))
            break;
          RIP = next_rip;
          BX_INFO(("poly_ud: emulated aarch64 %s x%u,x%u,x%u result=%llu", op_name, rd, rn, rm, (unsigned long long) result));
          return true;
        }
      }
      if (bx_poly_current_mode == BX_POLY_MODE_AARCH64 && (insn & 0xffc0fc00) == 0x8b000000) {
        ...
        if (rd != 0 || rn != 0 || rm != 1)
          break;
        RAX += bx_poly_aarch64_x[1];
        RIP = next_rip;
        BX_INFO(("poly_ud: emulated aarch64 add x0,x0,x1 value=%llu", (unsigned long long) bx_poly_aarch64_x[1]));
        return true;
      }
      ...
      if (bx_poly_current_mode == BX_POLY_MODE_AARCH64 && insn == 0x9b017c00) {
        RAX *= bx_poly_aarch64_x[1];
        RIP = next_rip;
        BX_INFO(("poly_ud: emulated aarch64 mul x0,x0,x1 value=%llu", (unsigned long long) bx_poly_aarch64_x[1]));
        return true;
      }
      if (bx_poly_current_mode == BX_POLY_MODE_AARCH64 &&
        ...
        RIP = next_rip;
        BX_INFO(("poly_ud: emulated aarch64 %s x0,x0,x1 value=%llu", op_name, (unsigned long long) bx_poly_aarch64_x[1]));
        return true;
      }
    ...
    case 0x26: {
      ...
      if (bx_poly_current_mode == BX_POLY_MODE_RISCV && (insn & 0x0000007f) == 0x00000033) {
        ...
        if (op_name != 0) {
          if (!write_poly_riscv_reg(rd, result))
            break;
          RIP = next_rip;
          BX_INFO(("poly_ud: emulated riscv %s x%u,x%u,x%u result=%llu", op_name, rd, rs1, rs2, (unsigned long long) result));
          return true;
        }
      }
      if (bx_poly_current_mode == BX_POLY_MODE_RISCV && (insn & 0xfe00707f) == 0x00000033) {
        ...
        if (rd != 10 || rs1 != 10 || rs2 != 11)
          break;
        RAX += bx_poly_riscv_x[11];
        RIP = next_rip;
        BX_INFO(("poly_ud: emulated riscv add a0,a0,a1 value=%llu", (unsigned long long) bx_poly_riscv_x[11]));
        return true;
      }
      ...
      if (bx_poly_current_mode == BX_POLY_MODE_RISCV && (insn & 0xfe00707f) == 0x02000033) {
        ...
        RAX *= bx_poly_riscv_x[11];
        RIP = next_rip;
        BX_INFO(("poly_ud: emulated riscv mul a0,a0,a1 value=%llu", (unsigned long long) bx_poly_riscv_x[11]));
        return true;
      }
      if (bx_poly_current_mode == BX_POLY_MODE_RISCV &&
      ...
      if (bx_poly_current_mode == BX_POLY_MODE_RISCV && (insn & 0x000fffff) == 0x00000893) {
        bx_poly_riscv_x[17] = (insn >> 20) & 0xfff;
        bx_poly_riscv_x_valid[17] = true;
        RIP = next_rip;
        BX_INFO(("poly_ud: emulated riscv addi a7,x0,%u", (unsigned) bx_poly_riscv_x[17]));
        return true;
      }
      if (bx_poly_current_mode == BX_POLY_MODE_RISCV && insn == 0x00000073) {
        Bit32u syscall_reg = (Bit32u) bx_poly_riscv_x[17];
        return handle_poly_foreign_syscall("riscv", "ecall", "a7=", syscall_reg,
          syscall_reg, syscall_reg, RAX, bx_poly_riscv_x[11], bx_poly_riscv_x[12],
          bx_poly_riscv_x[13], bx_poly_riscv_x[14], bx_poly_riscv_x[15], next_rip);
      }
